...
def criteria_with_params(pricedb,zipcodedb,fee_portal,due_difference, zipcode, client_data,due,common_db_data,portal_name,address):
    try:
        ...
        if adrs and adrs[0].isdigit():
                print("Address format True")
                
        
                if not client_data['miles']: client_data['miles'] = 20
                if not client_data['reps']: client_data['reps'] = 2
                ...
                if float(fee_portal) >= pricedb:
                    ...
                    if zipcodedb.get(str(zipcode),None) is not None :          #Convert to dictionary
                        if client_data['velocityClients_within_coverage_status'] == 'OFF':
                            ...
                            if (client_data.get('IsVelocityRangeCheck') and 
                                (client_data.get('min_rep', 0) != 0 and client_data.get('min_miles', 0) != 0) and 
                                (client_data.get('max_rep', 0) != 0 and client_data.get('max_miles', 0) != 0)):
                                reps = velocity_range_check(zipcode, client_data['Client_name'], portal_name, client_data['min_rep'], client_data['max_rep'], client_data['min_miles'], client_data['max_miles'])
                            else:
                                reps = velocity_check(zipcode,client_data['miles'],client_data['Client_name'],portal_name)
                            if reps >= client_data['reps']:
                        ...
                        zipcode=str(zipcode)
                        if client_data['velocityClients_status'] == 'ON':
                            if (client_data.get('IsVelocityRangeCheck') and 
                                (client_data.get('min_rep', 0) != 0 and client_data.get('min_miles', 0) != 0) and 
                                (client_data.get('max_rep', 0) != 0 and client_data.get('max_miles', 0) != 0)):
                                reps = velocity_range_check(zipcode, client_data['Client_name'], portal_name, client_data['min_rep'], client_data['max_rep'], client_data['min_miles'], client_data['max_miles'])
                                logging.info("exited velocity check")
                            else:
                                reps = velocity_check(zipcode,client_data['miles'],client_data['Client_name'],portal_name)
                ...
                ignored_msg = 'Address format not satisfied '
                return  ignored_msg,fee_portal,False     
...
def counter_accepted(client_data,address,portal,zipcode,ordertype,fee_portal):
    try:
        ...
        if data:
            ...
            if client_data['from_mail']== 'bangrealty@bpoacceptor.com':
                logging.info(f"from mail : {client_data['from_mail']}")
                ignormsg=ignored_message("Unable to process",address,'Already countered order',client_data['Client_name'],ordertype,fee_portal,zipcode)
                mail = sender.Mail('smtp.gmail.com', client_data['from_mail'] , email_creds[client_data['from_mail']], 465, use_ssl=True,fromaddr= client_data['from_mail'])
                client_mail_send(mail,client_data['to_clientMail'],client_data['to_ecesisMail'],subject,ignormsg)
...
def velocity_range_check(zip_code, client_name, portal, min_rep, max_rep, min_miles, max_miles):
    """This function is used to check whether reps are available in a specified range inVelocity"""
    logging.info("entry to velocity")
    max_retries = 2
    retry_delay = 0
    attempt = 0
    while attempt < max_retries:
        try:
            ...
            cursor.close()
            session=requests.session()
            min_miles_response=session.get('https://bpophotoflow.com/coverage_markers.php?lat={}&lng={}&radius={}'.format(lat,lng,min_miles))
            max_miles_response=session.get('https://bpophotoflow.com/coverage_markers.php?lat={}&lng={}&radius={}'.format(lat,lng,max_miles))
            min_miles_phtghr_count=0
            max_miles_phtghr_count= 0

            for child in ElementTree.fromstring(min_miles_response.content).iter('marker'):
                if child.attrib['color'] == 'blue.png':
                    min_miles_phtghr_count=min_miles_phtghr_count+1
            logging.info(f"min miles phtghr_count: {min_miles_phtghr_count} , zip_code:{zip_code}")

            for child in ElementTree.fromstring(max_miles_response.content).iter('marker'):
                if child.attrib['color'] == 'blue.png':
                    max_miles_phtghr_count=max_miles_phtghr_count+1
            logging.info(f"max miles phtghr_count: {max_miles_phtghr_count} , zip_code:{zip_code}")
            
            if min_miles_phtghr_count >= min_rep and max_miles_phtghr_count >= max_rep:
                return 2
            else:
                return 0
        except Exception as ex:
...
def inactive_inDB(client_name,portal):
    ...
    try:
        ...
        logging.info(err_message)
        if 'Bang-' in client_name :
            mail.send_message(subject=f'{portal}:- Inactive In Database!', to=('busdev@ecesistech.com',email_creds['internal_email_notification']), body=err_message)
        else:
...
Unable to login to {client_data['Client_name']}'s {portal_name} account"""
        ...
        if client_data['client_type']=="processing":
            if 'Bang' in client_data['Client_name']:
                mail = sender.Mail('smtp.gmail.com', email_creds['bang_login_error_email'] , email_creds['bang_login_error_password'], 465, use_ssl=True,
                                   fromaddr=email_creds['bang_login_error_email'])
    ...
    if typecheck_flag:
        ...
        if "Bang" in client_data['Client_name'] and inspectionflag == True:
            ...
            if zipcode not in common_db_data['bangnotavailablezip']:
                ...
                logging.info("Zip code in bangnotavailablezip")
                return False
        else:
